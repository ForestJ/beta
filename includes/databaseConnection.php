...
getDatabase(isset($_GET['cleandb']) ? "reset" : "get");

function getDatabase($command) {
...
END;
	...
	if($command == "reset" && array_key_exists('id', $lastUser) && $lastUser['id'] != "") {
		$q = "INSERT INTO `bfs`.`user` (`id`, `name`, `ip`, `time`, `network`, `version`, `updated`) "
		   . "VALUES ('" . $lastUser['id'] . "', '" . $lastUser['name'] . "', '" . $lastUser['ip'] . "', '0', 'release', '" . $version . "', 'false');";
		mysql_query($q) or die('Query failed: ' . mysql_error());
